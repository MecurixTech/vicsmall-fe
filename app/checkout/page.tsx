...
export default function CheckoutPage() {
  ...
  const [savedFormData, setSavedFormData] = useState<Partial<FormValues> | null>(null)
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      const orderData = {
        ...
        partPayment: checkoutData?.partPayment || 0,
      }
      ...
      <>
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="flex justify-center items-center h-[50vh]">
            ...
            <span className="ml-2 text-lg">Loading checkout information...</span>
          </div>
        </div>
      </>
    )
              ...
              <div className="space-y-6 rounded-lg border p-6">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span>N {checkoutData?.subtotal.toLocaleString() || "0"}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Delivery Fee</span>
                    <span>N {checkoutData?.deliveryFee.toLocaleString() || "0"}</span>
                  </div>
                  {(checkoutData?.discount || 0) > 0 && (
                    <div className="flex justify-between">
                      <span className="text-gray-500">Discount</span>
                      <span className="text-red-500">-N {checkoutData?.discount.toLocaleString() || "0"}</span>
                    </div>
                  )}
                  <hr className="my-4" />
                  <div className="flex justify-between font-semibold">
                    <span>Grand Total</span>
                    <span>N {checkoutData?.grandTotal.toLocaleString() || "0"}</span>
                  </div>
                  {checkoutData?.paymentMode === "part" && (
                    <div className="flex justify-between font-semibold text-green-600">
                      <span>Part Payment ({checkoutData.partPaymentPercentage}%)</span>
                      <span>N {checkoutData.partPayment.toLocaleString()}</span>
                    </div>
                  )}
                </div>
