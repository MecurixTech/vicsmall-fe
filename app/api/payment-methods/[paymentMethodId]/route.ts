...
export async function PATCH(request: NextRequest, { params }: { params: { paymentMethodId: string } }) {
  try {
    ...
    if (!result.success) {
      return NextResponse.json({ error: result.error || "Failed to update payment method" }, { status: 400 })
    }
...
export async function DELETE(request: NextRequest, { params }: { params: { paymentMethodId: string } }) {
  try {
    ...
    if (!result.success) {
      return NextResponse.json({ error: result.error || "Failed to delete payment method" }, { status: 400 })
    }
