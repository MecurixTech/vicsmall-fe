...
import { getCartItems, addToCart, removeFromCart, updateCartItemQuantity } from "@/lib/cart-service"
import type { CartItemResponse } from "@/lib/cart-service"
import { enrichCartItems, type EnrichedCartItem } from "@/utils/cart-product-utils"
import { calculateCartTotals, saveCartTotalsToStorage } from "@/utils/cart-calculations"
...
interface CartContextType {
  ...
  buyNow: (productId: string, quantity: number) => Promise<{ success: boolean; error?: string }>
  cartTotals: {
    subtotal: number
    deliveryFee: number
    discount: number
...
export const CartProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    if (Object.keys(productQuantities).length > 0) {
      try {
        localStorage.setItem(CART_QUANTITIES_KEY, JSON.stringify(productQuantities))
      } catch (error) {
        console.error("[CartContext] Error saving product quantities to localStorage:", error)
      }
    }
  }, [productQuantities])
      ...
      if (response.success) {
        ...
        if (updatedProductsRef.current.size > 0) {
          ...
          const updatedItems = response.data.map((serverItem) => {
            const existingItem = items.find((item) => item.product_id === serverItem.product_id)

            if (existingItem && updatedProductsRef.current.has(serverItem.product_id)) {
              return {
                ...serverItem,
        ...
        console.error("[CartContext] Failed to fetch cart items:", response.error)
        if (!silent) {
  ...
  const handleAddToCart = async (productId: string, quantity: number) => {
    ...
    try {
      ...
      if (response.data) {
        ...
        setItems((prevItems) => [...prevItems, newItem as CartItem])
  ...
  const handleUpdateQuantity = (productId: string, quantity: number) => {
    ...
    const enrichedItemIndex = enrichedItems.findIndex((item) => item.product_id === productId)
  ...
  const clearCart = async (): Promise<boolean> => {
    ...
    try {
      ...
      const removePromises = items.map((item) => handleRemoveFromCart(item.cart_id))
      ...
      toast.error("Error clearing your cart")
      return false
    } finally {
      setIsLoading(false)
    }
